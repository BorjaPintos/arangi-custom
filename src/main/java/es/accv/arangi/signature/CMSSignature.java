...
/**
 * Clase que representa una firma en formato CMS.<br><br>
 * 
 ...
 */
public class CMSSignature extends es.accv.arangi.base.signature.CMSSignature {
	 ...
	 */
	public CMSSignature(InputStream isSignature) throws NormalizeCertificateException, SignatureException, IOException {
		super(isSignature);
	}
	...
	/**
	 * Obtiene un objeto {@link CMSSignature CMSSignature} al firmar el documento con los
	 ...
	 */
	public static CMSSignature sign (ACCVDeviceManager[] managers, IDocument document) throws HashingException, LoadingObjectException, SignatureException {
		return sign(managers, document, false);
	}
	 ...
	 * @throws SignatureException Error durante el proceso de firma
	 */
	public static CMSSignature sign (ACCVDeviceManager[] managers, IDocument document, boolean isAttached) throws HashingException, LoadingObjectException, SignatureException {
		...
		byte[] cmsSignature;
		if (isAttached) {
			cmsSignature = createPKCS7CMS (signaturesBytes, certificates, document, algorithms, OID_FORMATO_FIRMA);
		} else {
			cmsSignature = createPKCS7CMS (signaturesBytes, certificates, null, algorithms, OID_FORMATO_FIRMA);
		}
		
		//-- Obtener el objeto
		try {
			return new CMSSignature (cmsSignature);
		} catch (NormalizeCertificateException e) {
			return null;
		}
	}
	 ...
	 * </ul>
	 * 
	 * @param document documento firmado en el CMS.
